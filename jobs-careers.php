<?php $title = "Jobs & Careers - Citizz Adzz"; ?>
		...
		<div class="container">
			<div class="row">
				<div class="col-xl-12 text-center">
					<h1 class="text-light">Jobs & Careers</h1>
					<div class="d-flex justify-content-center flex-wrap flex-md-nowrap align-items-center pt-3">
						<ol class="breadcrumb px-3 py-2">
							...
							<li class="breadcrumb-item fw-bold">Jobs & Careers</li>
						</ol>
					</div>
						...
						<div class="dropdown-center">
							...
							<?php
								...
								if(mysqli_num_rows($result) > 0)
								{
									...
									{
							?>
								<li><a class="dropdown-item" href="jobs-careers.php?id=<?php echo $row['cid'] ?>&name=<?php echo $row['cname'] ?>"><?php echo $row['cname'] ?></a></li>						
							<?php
									}
								}
							?>
		...
		<div class="container">			
			<?php
				if(isset($user_id, $user))
				{
					...
					{
			?>
			<h3 class="display-5">Jobs & Careers in <?php echo isset($_GET['name']) ? $_GET['name'] : '' ?></h3>
			<hr class="pb-3">
			<?php
						$id = $_GET['id'];					
						$select = "SELECT * FROM `jobs_careers` WHERE `jc_city` = '$id' AND `jc_status` = '1'";
						$result = mysqli_query($con, $select);
						if(mysqli_num_rows($result) > 0)
						{
			...
			<div class="row gy-3" d_data-masonry='{"percentPosition": true }'>
				...
				<div class="col-xl-4 col-lg-4 col-md-4 d-flex">
					<div class="card w-100">
						<div class="card-header">
							<a href="<?php echo $root ?>assets/img/jobs_careers/<?php echo $row['jc_img'] ?>" class="btn btn-sm btn-custom-gradient float-end" target="_blank">
								<i class="bi bi-file-earmark-pdf-fill me-2"></i>View PDF File
							</a>
						</div>
						<div class="card-body">
							<canvas id="pdf-canvas" url="<?php echo $root ?>assets/img/jobs_careers/<?php echo $row['jc_img'] ?>" class="img-fluid w-100 h-100" width="" height="250"></canvas>
						</div>						
					</div>					
				</div>
				<?php
				...
				?>
				<div class="col-xl-4 col-lg-4 col-md-4 d-flex">
					<a href="assets/img/jobs_careers/<?php echo $row['jc_img'] ?>" class="image-popup d-flex h-100">
						<img src="assets/img/jobs_careers/<?php echo $row['jc_img'] ?>" class="img-fluid shadow" lazyload>
					</a>
				</div>
				<?php
								}
