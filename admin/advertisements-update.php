...
<?php
	$id = $_GET['id'];
	$select = "SELECT * FROM `advertisements` WHERE `ad_id` = '$id'";
			...
			<div class="col-xl-12">
				<div class="card shadow-sm mb-3">
					<div class="card-header">
						<h5 class="card-title">Update Advertisements</h5>
					</div>
					<div class="card-body">
						<img src="../assets/img/advertisements/<?php echo $row['ad_img'] ?>" class="img-fluid" width="200">
						<form class="form row align-items-end" method="POST" enctype="multipart/form-data">
							<div class="col-xl-5 col-lg-4 col-md-4 mb-3">
								...
								<input type="file" class="form-control" name="ad_img" accept=".jpg, .jpeg, .pdf, .png, .gif">
							</div>
								...
								<select class="form-control" name="ad_city" required>
									...
									<?php
										...
										if(mysqli_num_rows($cresult) > 0)
										{
											...
											{
									?>
									<option value="<?php echo $crow['cid'] ?>" <?php echo $crow['cid'] == $row['ad_city'] ? 'selected' : '' ?>><?php echo $crow['cname'] ?></option>
									<?php
											}
										}
									?>
							...
							<div class="col-xl-2 col-lg-2 col-md-2 mb-3">
								...
								<select class="form-control" name="ad_status">
									<option value="1" <?php echo $row['ad_status'] == '1' ? 'selected' : '' ?>>Active</option>
									<option value="2" <?php echo $row['ad_status'] == '2' ? 'selected' : '' ?>>Inactive</option>
								</select>
							</div>
							<div class="col-xl-2 col-lg-2 col-md-2 mb-3">
...
<?php
	...
	if (isset($_POST['update']))
	{
		...
		if(!empty($_FILES['ad_img']['name']))
		{
			...
			if(file_exists($target_dir . basename($ad_img)))
			{
				echo ("<script LANGUAGE='JavaScript'>
			        alert('".$ad_img." already exists !');
			        window.location.href = '';
			    </script>");
			}
			else
			{
				if(move_uploaded_file($ad_img_tmp, $target_dir . basename($ad_img)))
				{
					unlink('../assets/img/advertisements/'.$row['ad_img']);
					$update = "UPDATE `advertisements` SET `ad_entry` = '$entry_date', `ad_city` = '$ad_city', `ad_img` = '$ad_img', `ad_status` = '$ad_status' WHERE `ad_id` = '$id'";
				}				
			}
		}
		else
		{
			$update = "UPDATE `advertisements` SET `ad_entry` = '$entry_date', `ad_city` = '$ad_city', `ad_status` = '$ad_status' WHERE `ad_id` = '$id'";
		}
		// echo $update; exit;
